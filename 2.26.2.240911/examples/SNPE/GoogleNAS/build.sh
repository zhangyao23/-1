...
helpMessage(){
cat << EOF
Usage: $(basename -- $0) [-h] [-q SDK_ROOT] [-c CERT_PATH] [-p PROJECT NAME] [-d DOCKER_NAME] [-r REGION] [-a ANDROID_NDK] [-o OUTPUT_DIR]
...
optional argument(s):
 -q SDK_ROOT                SDK ROOT, either provided explicitly or set to the SDK_ROOT environment variable.
...
do
  case "$opt" in
    ...
    c )
      if [[ -f "$OPTARG" ]]; then
        CERT_FILE=$OPTARG
        echo "Setting SSL cert to $OPTARG"
      fi
    ...
    q )
      SDK_ROOT=$OPTARG
      copy_sdk=1
      echo "Mapping SDK Root: $SDK_ROOT to /snpe/sdk"
      ;;
    ? )
      helpMessage;
      exit 1
...
if [[ -d "$SDK_ROOT" ]]; then
  if [[ $copy_sdk -eq 1 ]]; then
    echo "Copying $SDK_ROOT to 'sdk'"
    cp -r -L $SDK_ROOT /tmp/nas_build/sdk
  fi
fi
...
DOCKERFILE="$SDK_ROOT/examples/SNPE/GoogleNAS/Dockerfile"
if [[ -z "$SDK_ROOT" ]]
then
