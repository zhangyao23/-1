...
int main(int argc, char** argv) {
    ...
    static struct WinOpt::option long_options[] = {
      ...
      {"m",     WinOpt::required_argument,    NULL,  OPT_CONTAINER},
      {"i",     WinOpt::required_argument,    NULL,  OPT_INPUT_LIST},
      {"o",     WinOpt::required_argument,    NULL,  OPT_OUTPUT_DIR},
      {"b",     WinOpt::required_argument,    NULL,  OPT_BUFFER_TYPE},
      {"s",     WinOpt::no_argument,          NULL,  OPT_BUFF_SHARED},
      {"d",     WinOpt::no_argument,          NULL,  OPT_DMABUF_HEAP},
      {"q",     WinOpt::no_argument,          NULL,  OPT_STATIC_QUANTIZATION},
      ...
      {"x",     WinOpt::no_argument,          NULL,  OPT_FXP_CPU},
        ...
        switch (opt) {
            case 'h':
                std::cout
                        ...
                        << "  -d            Use libdmabufheap.so instead of libcdsprpc.so. Not supported on windows.\n"
            ...
            case 'd':
#ifdef _WIN32
                std::cout << "Using libdmabufheap.so is not supported on windows. Instead,"
                             " proceeding with libcdsprpc.so" << std::endl;
#else
                useRpc = false;
#endif
                break;
    ...
    if(usingSharedMemBuffers && (runtime != SNPE_RUNTIME_DSP)){
