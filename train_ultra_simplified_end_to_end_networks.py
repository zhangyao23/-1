...
def main():
    ...
    # 保存标准化器
    import joblib
    joblib.dump(scaler, 'ultra_simplified_raw_data_scaler.pkl')
    print("标准化器已保存为 'ultra_simplified_raw_data_scaler.pkl'")
    
    # 3. 训练异常检测网络（二分类）
    print("\n=== 训练超简化版异常检测网络（11维输入 → 2分类）===")
    ...
    print(f"超简化版异常检测网络参数数量: {sum(p.numel() for p in detector_model.parameters()):,}")
    ...
    print(f"超简化版异常检测网络最佳验证准确率: {best_val_acc:.2f}%")
    
    # 保存异常检测模型
    torch.save(detector_model.state_dict(), 'ultra_simplified_end_to_end_anomaly_detector.pth')
    print("超简化版异常检测模型已保存为 'ultra_simplified_end_to_end_anomaly_detector.pth'")
    
    # 4. 训练异常分类网络（仅异常样本）
    print("\n=== 训练超简化版异常分类网络（11维输入 → 6分类）===")
    ...
    print(f"超简化版异常分类网络参数数量: {sum(p.numel() for p in classifier_model.parameters()):,}")
    
    train_losses, val_losses, val_accuracies, best_val_acc = train_model(
        classifier_model, train_cls_loader, val_cls_loader, epochs=100, lr=0.001
    )
    
    print(f"超简化版异常分类网络最佳验证准确率: {best_val_acc:.2f}%")
    
    # 保存异常分类模型
    torch.save(classifier_model.state_dict(), 'ultra_simplified_end_to_end_anomaly_classifier.pth')
    print("超简化版异常分类模型已保存为 'ultra_simplified_end_to_end_anomaly_classifier.pth'")
    
    print("\n=== 训练完成 ===")
    print("✅ 两个超简化版端到端模型已成功训练并保存")
    print("✅ 模型现在可以直接处理11维原始网络监控数据")
    print("✅ 完全避免了复杂的特征工程和索引操作")
    print("✅ 应该可以完美转换为DLC格式")
